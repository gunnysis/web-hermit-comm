-- DB lint 에러 수정 (2차)
-- 1) get_my_streak: STABLE → VOLATILE (UPDATE 포함이므로)
-- 2) create_daily_post: 미사용 변수 v_deleted_existing 제거

-- ─── 1. get_my_streak: STABLE → VOLATILE ──────────────────────────────────────
...
COMMENT ON FUNCTION get_my_streak() IS 'daily 스트릭 조회 (연속/총/최장 + 마일스톤)';
